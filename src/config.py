...
class ModelConfig:
    ...
    max_len: int = 128           # For scratchpad sequences
    vocab_size: int = 21          # 16 → 21 (scratchpad tokens)
...
class TrainingConfig:
    ...
    num_samples: int = 150000       # CORRECTED: 500000 → 150000
    val_split: float = 0.05
    max_digits_supervised: int = 4  # CORRECTED: 3 → 4 (5-digit deferred)
    use_scratchpad: bool = True      # NEW flag
    batch_size: int = 64            # Reduced for longer sequences
    max_len: int = 128
    optimizer: str = "AdamW"
    ...
    warmup_steps: int = 400         # CORRECTED: 500 → 400
    min_lr: float = 3e-6
    epochs: int = 15               # CORRECTED: 30 → 15
    early_stopping: EarlyStoppingConfig = field(default_factory=EarlyStoppingConfig)
    criterion: str = "CrossEntropyLoss"
    ignore_index: int = 15          # PAD token ID (unchanged)
    gradient_accumulation_steps: int = 4  # CORRECTED: 4 (not 6)
...
class RLConfig:
    ...
    total_episodes: int = 10000       # Unchanged
    batch_size: int = 64
    use_model_config: bool = True
    learning_rate: float = 5e-6
    temperature: float = 1.0
    temperature_end: float = 0.6       # Lower final temp
    max_new_tokens: int = 40         # Scratchpad + answer
...
class EvaluationConfig:
    ...
    use_scratchpad: bool = False     # Evaluate without scratchpad
...
class PathsConfig:
    ...
    pretrained_model: str = "checkpoints/pretrained_v3.pth"
    rl_model: str = "checkpoints/rl_finetuned_v3.pth"
...
def load_config(config_path: str = "configs/hyperparams.yaml") -> Config:
    ...
    training = TrainingConfig(
        num_samples=int(train_cfg.get('num_samples', 150000)),
        val_split=float(train_cfg.get('val_split', 0.05)),
        max_digits_supervised=int(train_cfg.get('max_digits_supervised', 4)),
        use_scratchpad=bool(train_cfg.get('use_scratchpad', True)),
        batch_size=int(train_cfg.get('batch_size', 64)),
        max_len=int(train_cfg.get('max_len', 128)),
        ...
        warmup_steps=int(train_cfg.get('warmup_steps', 400)),
    ...
    rl = RLConfig(
        total_episodes=int(rl_cfg.get('total_episodes', 10000)),
        ...
        temperature_end=float(rl_cfg.get('temperature_end', 0.6)),
        max_new_tokens=int(rl_cfg.get('max_new_tokens', 40)),
        entropy_coef=float(rl_cfg.get('entropy_coef', 0.01)),
        entropy_coef_end=float(rl_cfg.get('entropy_coef_end', 0.005)),
        max_grad_norm=float(rl_cfg.get('max_grad_norm', 1.0)),
        replay_buffer=replay_buffer,
        curriculum=curriculum,
        dataset_sizes={k: int(v) for k, v in rl_cfg.get('dataset_sizes', {"easy": 15000, "medium": 15000, "hard": 30000}).items()},
        reward=reward,
    ...
    evaluation = EvaluationConfig(
        ...
        max_digits=eval_cfg.get('max_digits', 4),
        print_first_n=eval_cfg.get('print_first_n', 50),
        use_scratchpad=eval_cfg.get('use_scratchpad', False),
    ...
    paths = PathsConfig(
        ...
        pretrained_model=paths_cfg.get('pretrained_model', 'checkpoints/pretrained_v3.pth'),
        rl_model=paths_cfg.get('rl_model', 'checkpoints/rl_finetuned_v3.pth')
    )
...
if __name__ == "__main__":
    config = load_config()
    print("Loaded v3 config (corrected):")
